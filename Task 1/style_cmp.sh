...
then
    ...
    echo "$merge" > "OutPut.txt"

    ff=$(awk '{ print $1 , $4=($3*$3)-($2-$2)}' "OutPut.txt" )
    echo "$ff" > "OutPut.txt"
    
    f=$(awk '{ print $1 , $2=sqrt($2)}' "OutPut.txt"  )
    echo "$f" > "OutPut.txt"


    ss=$(awk '{ sum+=$2} END {print sum/27}' "OutPut.txt" )
    echo The Euclidian Distance between the two texts is: "$ss" >> "OutPut.txt"
